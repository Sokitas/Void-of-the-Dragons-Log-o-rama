...
class CombatAnalyzer:
    ...
    def analyze_hit(self, hit_lines):
        ...
        # Initialize base_damage and crit_enhanced_damage
        base_damage = player_damage
        crit_enhanced_damage = 0
        ...
        # Check if the hit is a crit and split the damage accordingly
        if "crit" in player_damage_line.lower():
            self.total_crits += 1
            # Calculate base_damage and crit_enhanced_damage
            base_damage = player_damage / (1 + self.crit_damage_bonus)
            crit_enhanced_damage = player_damage - base_damage

            # Add crit-enhanced damage to total crit damage
            self.total_crit_damage += (crit_enhanced_damage + base_damage)

        # Calculate total effect damage
        effect_damage = sum(effect['total_damage'] for effect in self.player_effects.values())
        ...
        if crit_enhanced_damage > 0:
            self.base_crit_damage += base_damage
            self.crit_enhanced_damage += crit_enhanced_damage
        ...
        self.hits_data.append({
            ...
            'crit_enhanced_damage': crit_enhanced_damage
        })

        if total_damage > self.personal_best_hit:
            self.personal_best_hit = total_damage
            self.save_config()
    ...
    def display_results(self):
        ...
        result_str = (
            ...
            f"Number of Crits {self.format_number(self.total_crits)}\n"
            f"Total Crit Damage: {self.format_number(self.total_crit_damage)}\n"
            f"Base Crit Damage: {self.format_number(self.base_crit_damage)}\n"
            f"Crit Enhanced Damage: {self.format_number(self.crit_enhanced_damage)}\n\n"
        ...
        for i, (effect, stats) in enumerate(self.player_effects.items()):
            ...
            proc_chance = (stats['procs'] / max(self.total_hits, 1)) * 100
            ...
            proc_chance_str = f"{proc_chance:.2f}%"
...
class CombatLogParserApp:
    def __init__(self, root):
        self.root = root
        root.title("Sokitas' Void of the Dragons Log-o-rama 2.4 - Who you gonna call? Logbusters!")
